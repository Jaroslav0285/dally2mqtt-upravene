...
void loop()
{
  ...
  static unsigned long wifiLost=0;
  static unsigned long lastHeartbeat=0;
  if (ESP.getFreeHeap() < 5000) { writeLog("<SYS > Low Heap"); ESP.restart(); }
  if (WiFi.status()!=WL_CONNECTED){
    if(!wifiLost) wifiLost=millis();
    if(millis()-wifiLost>60000){ writeLog("<SYS > WiFi timeout"); ESP.restart(); }
  } else wifiLost=0;
  if (bms.get.bmsHeartBeat>0) lastHeartbeat=millis();
  if(lastHeartbeat && millis()-lastHeartbeat>300000){ writeLog("<SYS > BMS timeout"); ESP.restart(); }
